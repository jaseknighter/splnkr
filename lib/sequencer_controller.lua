...
function sc.init()
    ...
    sc.lattice = Lattice:new{auto = true, meter = 1 / 2, ppqn = 96}
...
function sc.copy_paste_sequinsets(target_sequinset, source_sequinset)
    ...
    sc.sequins_outputs_table[target_sequinset] = fn.deep_copy(
                                                     sc.sequins_outputs_table[source_sequinset])
...
function sc.reset_sequinset_value_heirarcy(sgp, inner_table)
    ...
    if tab then
        for k, v in pairs(tab) do
            ...
            if k == "value_heirarchy" then v.sgp = sgp end
        end
    end
    local selected_sequin = sc.selected_sequin
    for i = 1, params:get("num_steps"), 1 do
...
function sc.print_outputs_table(inner_table)
    ...
    for k, v in pairs(tab_to_print) do
        if type(v) == "table" then sc.print_outputs_table(v) end
        if k == "value_heirarchy" then
            -- tab.print(v)
            -- print(">>>>>")
        end
...
sc.output_mode_map = {
    ...
    -- subsequins: step, num sub steps, starting sub step
...
sc.output_params_map = {
    ...
    }, -- effect (amp(nil), drywet(nil), pitchshift(nil), pitchshift offset(nil), pitchshift array (5)), phaser(nil), delay(nil), enveloper (3)
    {{nil, nil, nil}, {nil, nil, nil}, {nil, 4, nil, nil}} -- sequins: step, num steps, starting step
    -- subsequins: step, num sub steps, starting sub step
    -- clock/lattice/pattern: tempo, meter, pattern division
}
...
function sc.refresh_output_control_specs_map()
    ...
    sc.output_control_specs_map = {
            ...
            { -- midi note out 1-3 and stop/start
                {
                    ...
                    {
                        "option", {"1", "1/2", "1/4", "1/8", "1/16"}, 3, nil,
                        "dur", "dur"
                ...
                {
                    {"note", min_note, max_note, nil, "pitch", "pitch"},
                    {"number", '0.00', 10, nil, "level", "level"}
                }, -- play_note: pitch, level
                {
                    {"note", min_note, max_note, nil, "pitch", "pitch"},
                    {"number", '0.00', 10, nil, "level", "level"}
                }, -- play_voice channel 1: pitch, level
                {
                    {"note", min_note, max_note, nil, "pitch", "pitch"},
                    {"number", '0.00', 10, nil, "level", "level"}
                }, -- play_voice channel 2: pitch, level
                {
                    {"note", min_note, max_note, nil, "pitch", "pitch"},
                    {"number", '0.00', 10, nil, "level", "level"}
                }, -- play_voice channel 3: pitch, level
                {
                    {"note", min_note, max_note, nil, "pitch", "pitch"},
                    {"number", '0.00', 10, nil, "level", "level"}
                }, -- play_voice channel 4: pitch, level
                {
                    {"note", min_note, max_note, nil, "pitch", "pitch"},
                    {"number", '0.00', 10, nil, "level", "level"}
                }, -- play_voice channel 5: pitch, level
                {
                    {"note", min_note, max_note, nil, "pitch", "pitch"},
                    {"number", '0.00', 10, nil, "level", "level"}
                } -- play_voice channel 6: pitch, level
        ...
        }, { -- sequins: step, num steps, starting step
            -- subsequins: step, num sub steps, starting sub step
            -- clock/lattice/pattern: tempo, meter, pattern division
            { -- sequins      
                {"number", "1", 8, 1, "step", "step"},
                {"number", "1", 9, 9, "#seq", "num steps"},
                {"number", "1", 9, 1, "stseq", "starting seq"}
            }, { -- sub sequins      
                {"number", "1", 4, 1, "step", "sub step"},
                {"number", "1", 5, 5, "#seq", "num sub steps"},
            ...
            }, { -- clock/lattice/pattern
                {
                    ...
                    "option",
                    {"1/8", "1/4", "1/3", "1/2", "3/4", "1", "4/3", "3/2", "2"},
                    4, nil, "meter", "meter"
                }, {
                    "option",
                    {"1/8", "1/4", "1/3", "1/2", "3/4", "1", "4/3", "3/2", "2"},
                    6, nil, "pat_div", "pattern division"
                }
            }
        }
    }
...
function sc:unregister_ui_group(x1, y1)
    ...
    local selected_outputs = {
        sc.selected_sequin_output, sc.selected_sequin_output_mode,
        sc.selected_sequin_output_param, sc.active_sequin_value
    }
    if selected_outputs[y1 - 2] then selected_outputs[y1 - 2] = nil end
end
...
-- UI functions
function sc.get_active_sequinset_id() return sc.selected_sequin_group end

function sc.update_selected_sequin_group(index, state)
    sc.last_active_sequin_group = index
    sc.active_value_heirarchy = nil
    if state == "on" then
        local seq_ix = fn.deep_copy(sc.sequencers[sc.selected_sequin_group].seq
                                        .ix)
        local subseq_ix = selected_sub_sequin_ix and
                              fn.deep_copy(selected_sub_sequin_ix) or nil
        ...
        if seq_ix then
            ...
            seq()
            if subseq_ix then
                sc.sequencers[sc.selected_sequin_group].sub_seq_leader:select(
                    subseq_ix)
                sc.sequencers[sc.selected_sequin_group].sub_seq_leader()
            end
        end
        if sc.lattice.enabled == false then sc.lattice:start() end
    else
        if sc.lattice.enabled == true then sc.lattice:stop() end
        sc.selected_sequin_group = nil
...
-----------------------------
-- UI functions
function sc.set_selected_sequin(index) sc.selected_sequin = index end

function sc.set_ui_sequin_selector()
    if sc.sequin_selector == nil then
        sc.sequin_selector = grid_sequencer:register_ui_group("sequin_selector",
                                                              6, 1, 14, 1, 2, 3)
        local selected_sequin = sc.selected_sequin
        for i = 6, 14, 1 do
            for j = 1, 1, 1 do
        ...
        if selected_sequin then
            ...
            grid_sequencer:solid_on(selected_sequin + selected_sequin_offset, 1,
                                    1)
        end
    end
end
...
function sc.update_sequin_selector(x, y, state)
    if state == "on" then
        ...
        if sc.sequin_output_types == nil then
            sc.sequin_output_types = grid_sequencer:register_ui_group(
                                         "sequin_output_types", 6, 2,
                                         5 + num_output_types, 2, 4, 3)
        end
...
function sc.update_sequin_output_types(x, y, state)
    ...
    if state == "on" then
        ...
        sc.sequin_outputs = grid_sequencer:register_ui_group("sequin_outputs",
                                                             6, 3,
                                                             5 + num_outputs, 3,
                                                             7, 3)
    else
...
function sc.update_sequin_outputs(x, y, state)
    ...
    if state == "on" then
        ...
        local num_output_modes =
            sc.output_mode_map[output_type_selected][output_selected]
        local num_output_params =
            sc.output_params_map[output_type_selected][output_selected]
        ...
        if num_output_modes then -- the output selected has output_modes, show them
            sc.sequin_output_modes = grid_sequencer:register_ui_group(
                                         "sequin_output_modes", 6, 4,
                                         5 + num_output_modes, 4, 7, 3)
        elseif num_output_params then -- no output_modes, show the output params if there are any
            sc.sequin_output_params = grid_sequencer:register_ui_group(
                                          "sequin_output_params", 6, 5,
                                          5 + num_output_params, 5, 7, 3)
...
function sc.update_sequin_output_modes(x, y, state)
    ...
    if state == "on" then
        ...
        local num_output_mode_params =
            sc.output_params_map[output_type_selected] and
                sc.output_params_map[output_type_selected][output_selected] and
                sc.output_params_map[output_type_selected][output_selected][output_mode_selected]
        ...
        if num_output_mode_params then
            sc.sequin_output_params = grid_sequencer:register_ui_group(
                                          "sequin_output_params", 6, 5,
                                          5 + num_output_mode_params, 5, 7, 3)
        else
...
function sc.set_sequin_output_value_controls()
    ...
    if sc.output_control_specs_selected then
...
function sc.set_output_values(control_spec)
    ...
    if control_type == "note" then
        ...
        notes_per_octave = notes_per_octave < 12 and notes_per_octave or 11
        local num_octaves = math.ceil(params:get("scale_length") / notes_per_octave)

        local last_key = 5
        local first_key =
            num_octaves > last_key and 1 or last_key - num_octaves + 1
        local default_key = math.floor((last_key + first_key) / 2)
        sc.value_selector_notes = grid_sequencer:register_ui_group(
                                      "value_selector_notes", 6, 6,
                                      5 + notes_per_octave, 6, 7, 3,
                                      control_spec)
        sc.value_selector_octaves = grid_sequencer:register_ui_group(
                                        "value_selector_octaves", first_key, 6,
                                        last_key, 6, 4, 6, control_spec,
                                        default_key)
        if grid_sequencer:find_ui_group_num_by_name("selector_sequence_mode") == nil then
            sc.selector_sequence_mode = grid_sequencer:register_ui_group(
                                            "selector_sequence_mode", 4, 7, 5,
                                            7, 10, 6, control_spec, 5)
        end
    elseif control_type == "number" then
        ...
        control_max_length = polarity ~= -1 and
                                 #tostring(math.floor(control_max)) or
                                 control_min_length
        ...
        local decimal_num_places = decimal_location > 0 and control_min_length -
                                       decimal_location or 0
        ...
        integer_num_places = (control_max <= -1 or control_max >= 1) and
                                 integer_num_places or nil
        ...
        if decimal_location > 0 then
            value_place_decimals_x1 = decimal_num_places and 14 -
                                          decimal_num_places + 1 or nil
            value_place_decimals_x2 = decimal_num_places and 14 or nil
            sc.value_place_decimals = grid_sequencer:register_ui_group(
                                          "value_place_decimals",
                                          value_place_decimals_x1, 7,
                                          value_place_decimals_x2, 7, 4, 3,
                                          control_spec, control_default_index)
            sc.decimal_button = grid_sequencer:register_ui_group(
                                    "decimal_button",
                                    value_place_decimals_x1 - 1, 7,
                                    value_place_decimals_x1 - 1, 7, 15, 5)
        end

        local polarity_min_max = control_min < 0 and control_max > 0
        if polarity_min_max and sc.value_selector_polarity == nil then
            sc.value_selector_polarity =
                grid_sequencer:register_ui_group("value_selector_polarity", 4,
                                                 6, 5, 6, 4, 6, control_spec, 5)
        end
        ...
        if integer_num_places then
            ...
            if decimal_num_places and integer_num_places then
                value_place_integers_x1 =
                    14 - integer_num_places - decimal_num_places
                value_place_integers_x2 =
                    value_place_integers_x1 + integer_num_places - 1
            elseif integer_num_places then
                value_place_integers_x1 = 14 - integer_num_places
                value_place_integers_x2 =
                    value_place_integers_x1 + integer_num_places - 1
            end
            sc.value_place_integers = grid_sequencer:register_ui_group(
                                          "value_place_integers",
                                          value_place_integers_x1, 7,
                                          value_place_integers_x2, 7, 4, 3,
                                          control_spec, control_default_index)
    ...
    elseif control_type == "option" then
        ...
        sc.value_selector_options = grid_sequencer:register_ui_group(
                                        "value_selector_options", 6, 6,
                                        6 + num_options - 1, 6, 4, 3,
                                        control_spec, control_default_index)
        local existing_output_value = sc.get_active_output_table_slot()
                                          .output_value
        if existing_output_value == nil then
            -- clock.run(sc.activate_grid_key_at,6,6) 
        elseif existing_output_value then
            clock.run(sc.activate_grid_key_at,
                      5 + tonumber(existing_output_value.value), 6)
        end
    end
end
...
function sc.update_value_place_integers(x, y, state)
    ...
    if state == "on" then
        ...
        local selector_length = (is_last_integer_place and
                                    tonumber(last_place_value) > 0) and 5 +
                                    tonumber(last_place_value) or 14
        sc.value_selector_nums = grid_sequencer:register_ui_group(
                                     "value_selector_nums", 6, 6,
                                     selector_length, 6, 4, 3)
        local existing_output_value = sc.get_active_output_table_slot()
                                          .output_value
        if existing_output_value then
            local existing_output_value_int =
                tostring(math.floor(existing_output_value))
            ...
            local existing_output_value_at_place = string.sub(
                                                       ...
                                                       existing_output_value_int_length -
                                                           x_location + 1,
                                                       existing_output_value_int_length -
                                                           x_location + 1)

            if existing_output_value_at_place == "" then
                existing_output_value_at_place = 0
            end
            grid_sequencer.activate_grid_key_at(5 +
                                                    tonumber(
                                                        existing_output_value_at_place),
                                                6)
        end
    else
        -- sc:unregister_ui_group(6,6)
...
function sc.update_value_place_decimals(x, y, state)
    ...
    if state == "on" then
        ...
        local first_selector =
            (is_last_decimal_place and last_place_value > 0) and 5 +
                last_place_value or 6
        sc.value_selector_nums = grid_sequencer:register_ui_group(
                                     "value_selector_nums", first_selector, 6,
                                     14, 6, 4, 3)

        local selector_length =
            (is_last_decimal_place and last_place_value > 0) and 5 +
                last_place_value or 14
        -- sc.value_selector_nums = grid_sequencer:register_ui_group("value_selector_nums",6,6,selector_length,6,4,3)

        local existing_output_value = sc.get_active_output_table_slot()
                                          .output_value
        local decimal_location = existing_output_value and
                                     string.find(existing_output_value, "%.") or
                                     0

        local decimal_point_at = existing_output_value and
                                     string.find(existing_output_value, "%.")
        if existing_output_value and decimal_point_at then
            local existing_output_value_dec =
                tostring(string.sub(existing_output_value, decimal_point_at + 1))
...
function sc.update_value_selector_notes(x, y, state)
    if state == "on" then
        ...
        local selector_value = x - x_offset - 1
...
function sc.update_value_octave(x, y, state)
    if state == "on" then
        ...
        sc.sequin_output_values = grid_sequencer:register_ui_group(
                                      "sequin_output_values", 6, 8, 10, 8, 5, 3)
    end
end
    ...
    if press_type == "long" then -- >>
        ...
        if state == "on" then
            ...
            if sc.active_value_selector_place == "ten_thousands" then
                sc.active_sequin_value.place_values.ten_thousands =
                    selector_value
...
function sc.reset_place_values(exception)
    sc.active_sequin_value.place_values.ten_thousands = (exception ==
                                                            "ten_thousands") and
                                                            sc.active_sequin_value
                                                                .place_values
                                                                .ten_thousands or
    ...
    sc.active_sequin_value.place_values.thousands =
        (exception == "thousands") and
            sc.active_sequin_value.place_values.thousands or 0
    sc.active_sequin_value.place_values.hundreds =
        (exception == "hundreds") and
            sc.active_sequin_value.place_values.hundreds or 0
    sc.active_sequin_value.place_values.ones =
        (exception == "ones") and sc.active_sequin_value.place_values.ones or 0
    sc.active_sequin_value.place_values.tens =
        (exception == "tens") and sc.active_sequin_value.place_values.tens or 0
    sc.active_sequin_value.place_values.tenths =
        (exception == "tenths") and sc.active_sequin_value.place_values.tenths or
            0
    sc.active_sequin_value.place_values.hundredths =
        (exception == "hundredths") and
            sc.active_sequin_value.place_values.hundredths or 0
    sc.active_sequin_value.place_values.thousandths = (exception ==
                                                          "thousandths") and
                                                          sc.active_sequin_value
                                                              .place_values
                                                              .thousandths or 0
...
function sc.update_sequin_output_value(x, y, state, press_type)
    ...
    if press_type == "long" then
        local value_selector_group_id = grid_sequencer:find_ui_group_num_by_xy(
                                            6, 6)
        value_selector_default_value =
            grid_sequencer.ui_groups[value_selector_group_id].default_value
        value_selector_default_value = value_selector_default_value and
                                           value_selector_default_value or nil
    end
    ...
    if sc.active_sequin_value.value_type == "number" then
        ...
        if press_type == "long" then -- clear 
            ...
            output_value = sequence_mode == 1 and output_value .. "r" or
                               output_value
        end
    ...
    elseif sc.active_sequin_value.value_type == "notes" then
        if press_type == "long" then
            ...
            output_value = "-"
        elseif sc.active_sequin_value.note_value and
            sc.active_sequin_value.octave_value then
            local num_notes_per_octave = fn.get_num_notes_per_octave()
            local octave_offset = sc.active_sequin_value.octave_value *
                                      num_notes_per_octave
            local sequence_mode = sc.sequence_mode
    ...
    elseif sc.active_sequin_value.value_type == "option" then
        if press_type == "long" then
            output_value = value_selector_default_value and
                               value_selector_default_value or "-"
            sc.reset_place_values()
        else
...
function sc.update_outputs_table(output_value, output_sequins_index)
    ...
    if sc.sequins_outputs_table == nil then
        sc.sequins_outputs_table = {}
    end
    if sc.sequins_outputs_table[sgp] == nil then
        sc.sequins_outputs_table[sgp] = {}
        sc.sequins_outputs_table[sgp].table_type = "sgp"
    end
    if sc.sequins_outputs_table[sgp][ssg] == nil then
        sc.sequins_outputs_table[sgp][ssg] = {}
        sc.sequins_outputs_table[sgp][ssg].table_type = "sgp"
    end
    if sc.sequins_outputs_table[sgp][ssg][sqn] == nil then
        sc.sequins_outputs_table[sgp][ssg][sqn] = {}
        sc.sequins_outputs_table[sgp][ssg][sqn].table_type = "sqn"
    end
    if sc.sequins_outputs_table[sgp][ssg][sqn][typ] == nil then
        sc.sequins_outputs_table[sgp][ssg][sqn][typ] = {}
        sc.sequins_outputs_table[sgp][ssg][sqn][typ].table_type = "typ"
    end
    if sc.sequins_outputs_table[sgp][ssg][sqn][typ][out] == nil then
        sc.sequins_outputs_table[sgp][ssg][sqn][typ][out] = {}
        sc.sequins_outputs_table[sgp][ssg][sqn][typ][out].table_type = "out"
    end
    if sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod] == nil then
        sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod] = {}
        sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod].table_type =
            "mod"
    end
    ...
    if par == nil then
        ...
        if output_value and output_value ~= "clear" then
            if sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod]
                .output_data == nil then
                local num_outputs = sc.sequins_outputs_table[sgp][ssg][sqn]
                                        .num_outputs
                num_outputs = num_outputs == nil and 1 or num_outputs + 1
                sc.sequins_outputs_table[sgp][ssg][sqn].num_outputs =
                    num_outputs
                sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod]
                ...
                sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod]
                    .output_data = {}
                sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod]
                    .output_data.seq = Sequins {
                    table.unpack(DEFAULT_SUB_SEQUINS_TAB)
                }
            end
            sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod].table_type =
                "mod"
            ...
            if output_value ~= "-" then
                sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod]
                    .output_data.seq[output_sequins_index] = output_value
            else
                sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod]
                    .output_data.seq[output_sequins_index] = "nil"
            end
            ...
            sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod].output_data
                .control_id = sc.active_sequin_control_id
            sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod].output_data
                .control_name = sc.active_sequin_control_name
            sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod].output_data
                .value_type = sc.active_sequin_value.value_type
            sc.active_value_heirarchy = {
                sgp = sgp,
                ssg = ssg,
                sqn = sqn,
                typ = typ,
        ...
        if output_value and output_value ~= "clear" then
            if sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod][par]
                .output_data == nil then
                local num_outputs = sc.sequins_outputs_table[sgp][ssg][sqn]
                                        .num_outputs
                num_outputs = num_outputs == nil and 1 or num_outputs + 1
            ...
            sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod][par]
                .output_data.control_id = sc.active_sequin_control_id
            sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod][par]
                .output_data.control_name = sc.active_sequin_control_name
            sc.sequins_outputs_table[sgp][ssg][sqn][typ][out][mod][par]
                .output_data.value_type = sc.active_sequin_value.value_type
            sc.active_value_heirarchy = {
                sgp = sgp,
                ssg = ssg,
                sqn = sqn,
...
function sc.update_active_value_heirarchy()
    ...
    local output_data
    if a_map.par then
        output_data = (a_map.sqn and a_map.type and a_map.out and a_map.mod and
                          a_map.par) and
                          sc.sequins_outputs_table[a_map.sgp][a_map.ssg][a_map.sqn][a_map.typ][a_map.out][a_map.mod][a_map.par]
                              .output_data
        sc.active_value_heirarchy = {
            sgp = a_map.sgp,
            ssg = a_map.ssg,
    ...
    elseif a_map.mod then
        output_data = (sqn and type and out and mod and par) and
                          sc.sequins_outputs_table[a_map.sgp][a_map.ssg][a_map.sqn][a_map.typ][a_map.out][a_map.mod]
                              .output_data
        sc.active_value_heirarchy = {
            sgp = a_map.sgp,
            ssg = a_map.ssg,
...
function sc.refresh_selected_sequin_values(sgp, sqn, output_table,
                                           selected_sequin_values)
    sc.update_active_value_heirarchy()
    local output_table = output_table and output_table or
                             sc.sequencers[sgp].seq[sqn].active_outputs
    local sequin_id = sqn

    sc.selected_sequin_values = selected_sequin_values and
                                    selected_sequin_values or nil
    local selected_sequin_values = selected_sequin_values and
                                       selected_sequin_values or nil
    ...
    for k, v in pairs(output_table) do
        if k == "output_data" then
            ...
            if selected_sequin == sequin_id then
                ...
                if (sel_ixs.selected_sequin_group == sequin_group and
                    sel_ixs.selected_sequin_output_type == sequin_output_type and
                    sel_ixs.selected_sequin_output == sequin_output and
                    (sel_ixs.selected_sequin_output_param == nil or
                        sel_ixs.selected_sequin_output_param ==
                        sequin_output_param) and
                    (sel_ixs.selected_sequin_output_mode == nil or
                        sel_ixs.selected_sequin_output_mode ==
                        sequin_output_mode)) then
                    ...
                    if sc.output_control_specs_selected and
                        sc.output_control_specs_selected[1] == "option" then
                        for i = 1, #selected_sequin_values, 1 do
                            local idx = selected_sequin_values[i]
                            selected_sequin_values[i] =
                                sc.output_control_specs_selected[2][idx]
                        end
                    end
        ...
        elseif type(v) == "table" and selected_sequin_values == nil then
            sc.refresh_selected_sequin_values(sgp, sqn, v,
                                              sc.selected_sequin_values)
        end
    end
...
function sc.get_output_values(vh)
    ...
    if vh and vh.par then
        for i = 1, params:get("num_steps"), 1 do
            ...
            local sqn = (sgp and ssg) and
                            sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index]
            local type = (sgp and ssg and sqn) and
                             sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index][vh.typ]
            local out = (sgp and ssg and sqn and type) and
                            sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index][vh.typ][vh.out]
            local mod = (sgp and ssg and sqn and type and out) and
                            sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index][vh.typ][vh.out][vh.mod]
            local par = (sgp and ssg and sqn and type and out and mod) and
                            sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index][vh.typ][vh.out][vh.mod][vh.par]
            local output_data =
                (sgp and ssg and sqn and type and out and mod and par) and
                    sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index][vh.typ][vh.out][vh.mod][vh.par]
                        .output_data
            local output_value = (sqn and output_data) and output_data.value or
                                     nil
            output_value = output_value == nil and "nil" or output_value
            local calculated_absolute_value =
                (sqn and output_data and output_data.calculated_absolute_value) and
                    output_data.calculated_absolute_value or nil
            calculated_absolute_value = calculated_absolute_value == nil and
                                            "nil" or calculated_absolute_value
            table.insert(outputs_table,
                         {output_value, calculated_absolute_value})
        end
    elseif vh and vh.mod then
        for i = 1, params:get("num_steps"), 1 do
            ...
            local mod = (sgp and ssg and sqn and type and out) and
                            sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index][vh.typ][vh.out][vh.mod]
            local output_data =
                (sgp and ssg and sqn and type and out and mod) and
                    sc.sequins_outputs_table[vh.sgp][vh.ssg][sqn_index][vh.typ][vh.out][vh.mod]
                        .output_data
            local output_value = (sqn and output_data) and output_data.value or
                                     nil
