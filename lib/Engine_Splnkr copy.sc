...
Engine_Splnkr_copy : CroneEngine {
  ...
  *new { arg context, doneCallback;
  ...
  alloc {
    ...
    e1 = Buffer.loadCollection(context.server, h, 1);
    e2 = Buffer.loadCollection(context.server, h, 1);
    ...
    splnkrDef = SynthDef(\SplnkrSynth, {
      ...
      envBuf1, envBuf2, enveloper = 1, trigRate = 50, overlap = 0.5, panMax = 0.5,
      panType = 0, minGrainDur = 0.001, interpolation = 4,
      bufGate1=1,bufGate2=0;
      ...
      var gateTrig, gate, gates, signals, egs, mix;
      var sweep1, sweep2;
      ...
      // context.server.sync;
      envBuf1 = e1.bufnum * bufGate1;
      envBuf2 = e2.bufnum * bufGate2;

      numFrames = BufFrames.kr(envBuf1);
      // numFrames.poll;
     ...
     sweep2 = Sweep.ar(
      ...
      /*
      https://scsynth.org/t/how-to-get-rid-of-popping-sounds-when-jumping-around-playbuf/1671
      // could be Impulse or TDuty or other
      // gateTrig = Dust.kr(3);
      gateTrig = Impulse.kr(30);
      // one segment is on at first trigger, off at second
      gate = ToggleFF.kr(gateTrig);
      // the other is reversed
      gates = [gate, 1 - gate];

      // we need pairs of PlayBuf parameters, so: multichannel expansion
      // similarly, 2 PlayBufs defined above with alternating triggers
      // signals = PlayBuf.ar(1, b, trigger: gates, startPos: startFrames),
      signals = PlayBuf.ar(1, envs, trigger: gates);

      // alternating gate signals for 2 envelopes too
      egs = EnvGen.kr(Env.asr(1, 1, 1, [-4, 4]), gates);
      ...
      */
      ...
      // if enveloper is off, make sure the wet signal is still passed through
      // if enveloper is on, replace the wet signal with the enveloped signal
      wet = (Pan2.ar(wet * envs * amp, pan) * EnvGate.new * enveloper) + (wet*((enveloper+1)%2));
      // wet = (Pan2.ar(wet * mix * amp, pan) * EnvGate.new * enveloper) + (wet*((enveloper+1)%2));
    ...
    this.addCommand("set_env_times", "ffffffff", { arg msg;
      ...
      if (activeEnv == 2){
        ...
        e1 = Buffer.loadCollection(context.server, h, 1);
        activeEnv = 1;
        splnkrVoice.theSynth.set(\bufGate1, 1);
        splnkrVoice.theSynth.set(\bufGate2, 0);
      }{
        e1.free;
        ("e2").postln;
        e2 = Buffer.loadCollection(context.server, h, 1);
        activeEnv = 2;
        splnkrVoice.theSynth.set(\bufGate1, 0);
        splnkrVoice.theSynth.set(\bufGate2, 1);
      }
      */
