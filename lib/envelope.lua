...
function Envelope:new(id, num_envelopes, env_nodes)
  local e = {}
  e.index = 1
  ...
  e.init = function(num_envelopes)
    ...
    local graph_x = 10 + screen_size.x/num_envelopes*(e.id-1)
    local graph_y = 10
    local graph_width = screen_size.x/num_envelopes - 10
  ...
  e.set_env_time = function(env_time_max)
    ...
    if initializing == false then 
      params:set("envelope" .. active_envelope .. "_max_time", e.env_time_max) 
    end
    e.update_envelope()
  end
  ...
  e.remove_node = function()
    if (e.active_node > 1 and e.active_node < #e.graph_nodes and #e.graph_nodes > 3) then
      ...
      e.graph:edit_graph(e.graph_nodes)
      params:set("num_envelope" .. e.id .. "_controls",#e.graph_nodes)
    end
  end
  --------------------------
  -- encoders and keys
    ...
    elseif n == 3 then 
      if show_env_mod_params then
        ...
        elseif e.active_node_param == 1 then
          ...
          local param_id = id == 1 and envelope1_times[graph_active_node] or envelope2_times[graph_active_node]
        ...
        elseif e.active_node_param == 2 then
          if e.active_node ~= 1 and e.active_node ~= #e.graph_nodes then
            ...
            local param_id = id == 1 and envelope1_levels[graph_active_node] or envelope2_levels[graph_active_node]
            params:set(param_id,new_val)
          end
        ...
        e.graph:edit_graph(e.graph_nodes)
        -- reset_envelope_control_params(e.id)
  ...
  e.redraw = function ()
    ...
    elseif screen_dirty == true then
      ...
      if show_env_mod_params and active_envelope == e.id then e.draw_modulation_nav() end
